...
export default function FullFeaturedCrudGrid() {
  ...
  const handleSaveClick = (id) => () => {
    ...
    fetch(`http://localhost:5000/api/dell/${editedRow.Seller}`, {
      method: 'PUT',
      headers: {
  ...
  const fetchData = () => {
    fetch('http://localhost:5000/api/dell')
      .then((response) => response.json())
      .then((data) => {
        const rowsWithIds = data.dellData.map((row) => ({ id: uuidv4(), ...row }));
        setRows(rowsWithIds);
      })
      .catch((error) => {
  ...
  const handleDeleteClick = (id) => () => {
    ...
    if (deletedRow) {
      ...
      fetch(`http://localhost:5000/api/dell/${Seller}`, {
        method: 'DELETE',
      })
        .then((response) => response.json())
        .then((data) => {
